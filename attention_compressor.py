...
class AttentionCompressor:
    ...
    def _load_attention_model(self):
        ...
        # Raw-attention path uses the same last-token capture when use_all_queries=False
        # to avoid materializing full B×H×S×S tensors across all layers (CUDA OOM on long context).
        if (not self.use_raw_attention) or (not self.use_all_queries):
                        ...
                        if len(outputs) > 1 and outputs[1] is not None:
                            ...
                            if hasattr(attn, 'shape') and layer_idx in selected_heads:
                                for h in selected_heads[layer_idx]:
                                    if h < attn.shape[1]:
                                        # .clone() detaches from the [B,H,S,S] tensor — see
                                        # _patch_model_for_last_token for the full reason.
                                        compressor_ref.attn_tap.append((layer_idx, h, attn[:, h, -1, :].clone()))
                    ...
                    def patched_forward(self, *args, **kwargs):
                        ...
                        if len(outputs) > 1 and outputs[1] is not None:
                            ...
                            if hasattr(attn, 'shape'):
                                ...
                                # .clone() detaches from the underlying [B,H,S,S] tensor so it can
                                # be freed when outputs[1] is nullified below — otherwise the view
                                # holds a reference and full attentions accumulate across layers,
                                # causing CUDA OOM on long contexts.
                                last_token_attn = attn[:, :, -1, :].clone()
    ...
    def _raw_attention_filtering(
        ...
        # Tokenize. Truncate at self.max_seq_len so segmentation matches the
        # token range the model will actually attend over — without this,
        # _split_into_sentences would see context sentences past the truncation
        # point and emit indices that downstream attention can't address.
        ...
        with torch.no_grad():
            if not self.use_all_queries and hasattr(self, 'attn_tap'):
                # Memory-efficient path: rely on the monkey-patched layers (set up in
                # _patch_model_for_last_token) to capture only [B, H, S] slices and free
                # the full attention tensor inside each layer.
                self.attn_tap.clear()
                ...
                tap = sorted(self.attn_tap, key=lambda x: x[0])  # [(layer_idx, [B, H, S])]
                if self.use_last_layer_only:
                    tap = [tap[-1]]
                # Stack to [num_layers, H, S] (B=1)
                all_layers_attentions = torch.stack([t[1][0] for t in tap], dim=0)
                all_layers_attentions = all_layers_attentions[:, :, context_start:context_end+1]
                self.attn_tap.clear()
            else:
                ...
                # Extract relevant attention (use_all_queries=True path)
    ...
    def _detector_based_filtering(
        ...
        # Tokenize (truncated; see the matching note in _raw_attention_filtering).
        inputs = self.tokenizer(
            prompt, return_tensors="pt", return_offsets_mapping=True,
            truncation=True, max_length=self.max_seq_len,
        ).to(self.device)
    ...
    def _split_into_sentences(
        ...
        # Get sentence token ranges. Some sentences may have no overlapping
        # tokens (e.g. truncation cut them off, or the segmenter produced a
        # sentence whose characters tokenize to nothing — happens on certain
        # Unicode-heavy LongBench contexts). Mark those as None and drop in
        # the validity filter below.
        sent_positions = []
        for sent_idx in range(len(sentences)):
            sent_tokens = token_sentence_pairs[token_sentence_pairs[:, 1] == sent_idx, 0]
            if sent_tokens.size == 0:
                sent_positions.append(None)
                continue
